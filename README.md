...
FROM (
  ...
  SELECT
    b.no_seq AS `No`,
    13 AS bt,
    CONCAT( ((CEIL(b.no_seq / 50) - 1) % 999) + 1 , 'B') AS `bat`,  -- ★1..999でループ
    99 AS `個数`,
    ((b.no_seq - 1) % 50) + 1 AS `採番値(1~50)`,                   -- ★1..50でループ
  ...
  SELECT
    b.no_seq AS `No`,
    13 AS bt,
    CONCAT( ((CEIL(b.no_seq / 50) - 1) % 999) + 1 , 'B') AS `bat`,
    1 AS `個数`,
    ((b.no_seq - 1) % 50) + 1 AS `採番値(1~50)`,
    DATE_FORMAT(b.work_day, '%Y%m%d') AS `日`,
    b.nihno AS `SHNo`,
    b.mmno  AS `MMNO`,
    1 AS row_type
